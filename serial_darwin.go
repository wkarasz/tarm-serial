...
func openPort(name string, baud int, databits byte, parity Parity, stopbits StopBits, readTimeout time.Duration) (p *Port, err error) {
	...
	t := unix.Termios{
		Iflag:  unix.IGNPAR,
		Cflag:  uint64(cflagToUse), //WJK castcflagToUse to uint64
		Ispeed: uint64(rate), //WJK cast rate to uint64
		Ospeed: uint64(rate), //WJK cast rate to uint64
	}
	t.Cc[unix.VMIN] = vmin
	...
	if _, _, errno := unix.Syscall6(
		unix.SYS_IOCTL,
		uintptr(fd),
		uintptr(unix.TIOCSETA), //WJK uintptr(unix.TCSETS),
